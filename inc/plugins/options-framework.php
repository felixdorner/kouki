...
function kouki_theme_styles() { ?>
	...
	<style type="text/css" media="screen">
		...
		<?php if ($meta_color) : ?>
			...
			hr {
				background-color: <?php echo esc_attr($meta_color); ?>;
			}
		<?php endif; ?>
		...
		<?php $primary_font_weight = of_get_option('kouki_primary_font_weight'); ?>
		<?php if ($primary_font_weight) : ?>
			body, h2.meta, .comment-respond h3 small, button, input, select, textarea, caption, td {
				font-weight: <?php echo esc_attr($primary_font_weight); ?>;
			}
		<?php endif; ?>
		...
		<?php $secondary_font_weight = of_get_option('kouki_secondary_font_weight'); ?>
		<?php if ($secondary_font_weight) : ?>
			h1, h2, h3, h4, h5, h6, b, strong, blockquote p, table thead th,
			.widget-title, ul.wp-block-latest-posts li > a  {
				font-weight: <?php echo esc_attr($secondary_font_weight); ?>;
			}
		<?php endif; ?>
 ...
 * if the Options Framework plugin has been disabled.
 */
